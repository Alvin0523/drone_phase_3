...
This ROS noetic workspace is configured to use [Pixi](https://pixi.sh) for dependency management.
...
1. **Use a custom channel** — re-run init with `--channel` if the package lives elsewhere:
   ```bash
   pixi ros init --distro noetic --channel https://prefix.dev/my-channel
   ```
...
- [ROS noetic documentation](https://docs.ros.org/en/noetic/)
